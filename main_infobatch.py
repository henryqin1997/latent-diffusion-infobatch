...
#             self.datasets['train'] = InfoBatch(self.datasets['train'], num_epoch=1000, delta = 0.825)
    ...
    def _train_dataloader(self, trainer):
        ...
        print(f"{self.datasets['train'].__class__.__name__}, {len(self.datasets['train'])}")
...
#                 trainer.fit(model,train_dataloaders=train_dataloader,val_dataloaders=val_dataloader)
                trainer.validate(model, val_dataloader=val_dataloader)
            except Exception:
                melk()
                raise
